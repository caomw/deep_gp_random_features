...
class DgpRff(object):
    def __init__(self, likelihood_fun, num_examples, d_in, d_out, n_layers, n_rff, df, kernel_type, kernel_arccosine_degree, is_ard, feed_forward, q_Omega_fixed, theta_fixed, learn_Omega):
        ...
        ## When Omega is fixed, fix some standard normals throughout the execution that will be used to comstruct Omega
        if self.learn_Omega == True:
            self.get_kl = self.get_kl_Omega_to_learn
            self.sample_from_Omega = self.sample_from_Omega_to_learn

        if self.learn_Omega == False:
    ...
    ## Returns samples from prior over Omega - in the case where q(Omega) is not to be learned, the randomness is fixed throughout the optimization (and Monte Carlo samples)
